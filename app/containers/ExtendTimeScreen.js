...
class ExtendTimeScreen extends React.Component {
  ...
  _onExtendTime() {
    ...
    if (
      !isNumber(this.state.hour) ||
      !isNumber(this.state.minute) ||
      (parseInt(this.state.hour) == 0 && parseInt(this.state.minute) == 0)
    ) {
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.current_time_view}>
          <Text style={styles.current_time_text}>
            Your time : {this.state.hour} Hours {this.state.minute} Minutes
          </Text>
        </View>
  ...
  extendTimeWS() {
    ...
    let seconds =
      parseInt(this.state.hour) * 3600 + parseInt(this.state.minute) * 60;
    ...
    extendTime(params)
      .then(data => {
        ...
        if (data.errors) {
          ...
          console.log(
            "before timeLimit",
            storestate.tour.bookingdata.timeLimit
          );
          storestate.tour.bookingdata.timeLimit =
            storestate.tour.bookingdata.timeLimit +
            parseInt(this.state.hour) * 60 * 60 +
            parseInt(this.state.minute) * 60;
